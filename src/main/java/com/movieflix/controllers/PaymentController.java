...
public class PaymentController {
	...
	public String paymentSuccess(HttpSession session) {
		...
		User user = service.getUser(email);// Retrieves the user based on the email
		user.setPremium(true);// Sets the user's premium status to true
		service.updateUsers(user);// Updates the user in the database
		return "login";// Returns the view name "login"
	}

	// payment-failure -> redirect to login
	@GetMapping("payment-failure")// Maps HTTP GET requests to /payment-failure to this method
	public String paymentFailure() {// Method to handle GET requests to /payment-failure
		// payment-error page
		return "login"; // Returns the view name "login"
	}
